...
public class CheepRepository : ICheepRepository
{
    ...
    public Author CreateAuthor(string name, string email)
    {
        ...
        {
            AuthorId = _context.Authors.Count() + 1, Name = name, Email = email, Cheeps = new List<Cheep>()
        };
    ...
    public void UnFollow(string user, string unfollowing)
    {
        ...
        var query = (from Follows in _context.Following
            where Follows.User.AuthorId == AuthorUser.AuthorId &&
                  Follows.Following.AuthorId == AuthorUnfollowing.AuthorId
            select Follows);
    ...
    {
        var author = _context.Authors.SingleOrDefault(a => a.Email.ToLower() == email.ToLower());
        if (author == null)
        {
        ...
        var follows = _context.Following
            .Where(f => f.User.AuthorId == author.AuthorId || f.Following.AuthorId == author.AuthorId).ToList();
    ...
    public List<AuthorDTO> GetBlockedAuthors(string userEmail)
    {
        var query = (from Author in _context.Authors
            ...
            select new AuthorDTO { Name = Blocked.BlockedUser.Name, Email = Blocked.BlockedUser.Email, });
        return query.ToList();
    }
